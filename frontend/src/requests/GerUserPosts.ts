...
export const GetUserPosts = async (userId: string, page?: number): Promise<{data: postSummary[], totalPage: number}> => {
    if (USE_DUMMY) return dummy;

    const res: any = await fetch(`${REQUEST_URL}/user/index/${userId}?page=${page??1}`)
        .then(res => res.json());
