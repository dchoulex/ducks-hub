...
export const GetUserBookmarkPosts = async (userId: string, page?: number): Promise<{data: postSummary[], totalPage: number}> => {
    if (USE_DUMMY) return dummy;

    const res = await fetch(`${REQUEST_URL}/user/bookmark/${userId}?page=${page??1}`)
        .then(res => res.json());
